...
if ( ! function_exists( 'ppa_register_settings_api_bootstrap' ) ) {                                      // CHANGED:
        function ppa_register_settings_api_bootstrap() {                                                 // CHANGED:
                ...
                // - our Settings screen (page=postpress-ai-settings)
                ...
                $is_opts  = ( 'options.php' === $pagenow ) || ( 'options.php' === $phpself );            // CHANGED:
                $is_ppa   = ( 'postpress-ai-settings' === $page );                                       // CHANGED:

                if ( ! $is_opts && ! $is_ppa ) {                                                         // CHANGED:
                        return;                                                                          // CHANGED:
                }                                                                                        // CHANGED:
...
if ( ! function_exists( 'ppa_register_admin_menu' ) ) {
  ...
  <g stroke="#ffffff" stroke-width="1.8" stroke-linecap="round" fill="none">
    ...
    <path d="M7 11 L7 9"/>
                ...
                // Testbed page: hidden by default. If someone hits the URL directly while disabled, renderer returns "No access." // CHANGED:
...
if ( ! function_exists( 'ppa_render_testbed' ) ) {
        function ppa_render_testbed() {
                ...
                // CHANGED: Testbed must be explicitly enabled — even direct URL access is blocked otherwise.
                $testbed_enabled = ( defined( 'PPA_ENABLE_TESTBED' ) && true === PPA_ENABLE_TESTBED );            // CHANGED:
